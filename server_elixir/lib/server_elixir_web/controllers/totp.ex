defmodule ServerElixir.Controllers.MultiFactorAuthentication do
  ...
  def create(conn, _params) do
    ...
    case Authentication.update_user(user, %{two_factor_secret: secret}) do
      {:ok, user} ->
        uri = NimbleTOTP.otpauth_uri(user.email, user.two_factor_secret, issuer: Application.get_env(:server_elixir, :totp_key_name))
        conn
        |> put_status(:ok)
